...
class ImprovedDQN(nn.Module):
    def __init__(self, input_channels, action_dim):
        ...
        self.skip = nn.Conv2d(input_channels, 128, kernel_size=1)
    ...
    def forward(self, x):
        ...
        x = torch.relu(self.conv3(x) + skip_connection)
        x = nn.functional.adaptive_avg_pool2d(x, (12, 12))
...
class ImageFusionEnv:
    ...
    def calculate_reward(self, fusedImage):
        ...
        ssim_score = ssim(sq_im1, fusedImageCpu, data_range=255) + ssim(sq_im2, fusedImageCpu, data_range=255)
        
        laplacian = cv2.Laplacian(fusedImageCpu, cv2.CV_64F)
        sharpness_score = laplacian.var() / 100.0
        return ssim_score + sharpness_score

class ImprovedDQNAgent:
    def __init__(self, input_channels, action_dim):
        self.model = ImprovedDQN(input_channels, action_dim).to(device)
        self.target_model = ImprovedDQN(input_channels, action_dim).to(device)
...
def main():
    ...
    agent = ImprovedDQNAgent(input_channels=1, action_dim=3)
    ...
    agent.save_model('../SavedModels/dqn_improved_model.pth')
